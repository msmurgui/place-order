...
const buildReq = (body: unknown): Request => ({ body }) as Request;
...
describe('rateLimiter', () => {
  it('calls next when under the limit', async () => {
    ...
    await rateLimiter(buildReq({ customerId: 1 }), res, next);
  ...
  it('skips rate limiting (and Redis) when customerId is missing', async () => {
    const res = buildRes();

    await rateLimiter(buildReq({}), res, next);
  ...
  it('keys the limit on customerId', async () => {
    vi.mocked(redisClient.eval).mockResolvedValue(1);

    await rateLimiter(buildReq({ customerId: 42 }), buildRes(), next);

    expect(vi.mocked(redisClient.eval).mock.calls[0]).toContain('ratelimit:42');
  });
});
