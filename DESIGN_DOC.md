# Plan: POST /orders — Production-Ready Order Endpoint

## Overview

This service exposes a single `POST /orders` endpoint for an e-commerce platform. It accepts a customer's order (shipping address, items, and payment details), selects the optimal warehouse to fulfil the order, reserves the required inventory atomically, applies jurisdiction-based tax, processes payment, and returns a confirmed order with a full receipt snapshot.

Key guarantees:
- **Inventory safety** — append-only reservation table with per-SKU PostgreSQL advisory locks prevents overselling under concurrent load.
- **Idempotency** — duplicate submissions (retries, double-clicks, network timeouts) return the original response without re-charging.
- **Financial consistency** — payment charges that cannot be immediately confirmed land in a `PENDING_PAYMENT` state and are reconciled by a background job rather than being incorrectly failed.
- **Resilience** — circuit breakers on all external gateways, retry logic for transient failures, and a dead-letter queue for unresolvable compensation failures.

---

## Contents

- [Context](#context)
- [Tech Stack](#tech-stack)
- [Architecture — Four Layers](#architecture--four-layers)
- [Project Structure](#project-structure)
- [Database Schema](#database-schema)
- [Temporal Data Integrity — Order Snapshots](#temporal-data-integrity--order-snapshots)
- [Inventory Management System](#inventory-management-system)
- [Tax Gateway](#tax-gateway)
- [Layer Responsibilities in Detail](#layer-responsibilities-in-detail)
- [Compensating Transactions + Dead Letter Queue](#compensating-transactions--dead-letter-queue)
- [Idempotency](#idempotency)
- [Circuit Breakers](#circuit-breakers)
- [Type Safety](#type-safety)
- [Observability](#observability)
- [Background Jobs](#background-jobs)
- [Demo & Load Testing](#demo--load-testing)
- [Verification](#verification)
- [Implementation](#implementation)

---

## Context

Standalone endpoint for warehouse-based order fulfillment. A customer submits an order (shipping address + items + credit card); the server selects the closest warehouse that can fulfill all items, reserves inventory atomically via an **append-only reservation table**, charges the card inline, and returns the confirmed order.

> The end-to-end order placement flow is detailed step by step in [docs/order-flow.md](docs/order-flow.md).

---

## Tech Stack

| Layer | Choice | Reason |
|---|---|---|
| Runtime | Node 24 + TypeScript | Strict types; modern async |
| Framework | Express 5 | Minimal, well-understood |
| Database | PostgreSQL 16 | ACID transactions |
| ORM / Migrations | TypeORM | Entities, relations, query builder, and migrations in one package |
| Cache / store | Redis (ioredis) | Geocode cache; idempotency store (atomic SET NX claim); circuit-breaker flags |
| Concurrency control | PostgreSQL advisory locks | `pg_advisory_xact_lock` per `(warehouseId, productId)` — released atomically at commit, no TTL race |
| Background jobs | BullMQ | Reservation expiry, fulfillment, and order reconciliation workers — built on Redis (no extra infra) |
| Validation | Zod | Runtime schema + inferred types |
| Testing | Vitest + Supertest | Fast, ESM-native; integration tests via Supertest |

---

## Architecture — Four Layers

The application is divided into four strict layers. Each layer may only call the layer directly below it. No layer may skip layers.

```
┌─────────────────────────────────────────┐
│           Route Layer                   │  Rate limiting, schema validation,
│       src/routes/                       │  calls controller
├─────────────────────────────────────────┤
│         Controller Layer                │  Idempotency check, thin HTTP adapter
│       src/controllers/                  │  — orchestrates one service call,
│                                         │  maps result to HTTP response.
├─────────────────────────────────────────┤
│          Service Layer                  │  All business logic. Facade pattern:
│        src/services/                    │  OrderService is the single entry
│                                         │  point; internally coordinates
│                                         │  sub-services. No DB access.
├─────────────────────────────────────────┤
│          Data Layer                     │  Repositories only. All TypeORM
│       src/repositories/                 │  queries live here. No business
│                                         │  logic.
└─────────────────────────────────────────┘
```

### Why a single service entry point (Facade)

The controller calls `OrderService.createOrder()` only — it does not coordinate individual services like `WarehouseSelectionService` or `ReservationService` directly. This means:

- The controller is a dumb HTTP adapter. Adding a fraud check, loyalty points, or a notification step requires zero controller changes.
- `OrderService.createOrder()` is fully testable without HTTP context.
- When sub-services are eventually extracted into microservices, only `OrderService` changes — the controller and route are untouched.

---

## Project Structure

```
src/
├── config/
│   └── env.ts                              ← typed env vars (DB_URL, REDIS_URL, etc.)
│
├── db/
│   ├── dataSource.ts                       ← TypeORM DataSource singleton
│   ├── migrations/                         ← hand-written, versioned, one per table
│   │   ├── 001_create_customers.ts
│   │   ├── 002_create_products.ts
│   │   ├── 003_create_warehouses.ts
│   │   ├── 004_create_inventories.ts
│   │   ├── 005_create_orders.ts
│   │   ├── 006_create_order_items.ts
│   │   └── 007_create_inventory_reservations.ts
│   └── seeds/                              ← test fixture data
│
├── entities/                               ← TypeORM entities (schema source of truth)
│   ├── Customer.ts
│   ├── Product.ts
│   ├── Warehouse.ts
│   ├── Inventory.ts
│   ├── InventoryReservation.ts
│   ├── Order.ts
│   └── OrderItem.ts
│
├── repositories/                           ← DATA LAYER: all DB + Redis queries
│   ├── OrderRepository.ts
│   ├── OrderItemRepository.ts
│   ├── ProductRepository.ts
│   ├── WarehouseRepository.ts
│   ├── InventoryRepository.ts
│   ├── ReservationRepository.ts
│   └── IdempotencyRepository.ts            ← Redis-backed
│
├── services/                               ← SERVICE LAYER: all business logic
│   ├── OrderService.ts                     ← Facade entry point
│   ├── WarehouseSelectionService.ts
│   ├── ReservationService.ts               ← create / confirm / release reservations (advisory-locked)
│   └── GeocodeService.ts                   ← Redis cache wrapper around GeocodingGateway
│
├── controllers/                            ← CONTROLLER LAYER: thin HTTP adapter
│   └── orderController.ts
│
├── routes/                                 ← ROUTE LAYER: middleware pipeline
│   └── orders.ts
│
├── middleware/
│   ├── rateLimiter.ts                      ← per-IP sliding window, Redis-backed
│   └── circuitBreaker.ts                   ← Redis-backed boolean flags per gateway
│
├── gateways/
│   ├── GeocodingGateway.ts                 ← mock: address → { lat, lng }
│   ├── PaymentGateway.ts                   ← mock: charge card → { reference }
│   └── TaxGateway.ts                       ← mock: calculate tax per line item → { totalTaxAmount, breakdown }
│
├── jobs/
│   ├── expireReservations.ts               ← BullMQ worker: release stale reservations
│   ├── fulfillReservations.ts              ← BullMQ worker: roll confirmed reservations into physical stock
│   └── reconcileOrders.ts                  ← BullMQ worker: resolve PENDING_PAYMENT orders
│
├── util/
│   ├── haversine.ts
│   ├── retry.ts
│   └── logger.ts                           ← structured JSON logging (pino)
│
├── demo/                                   ← demo + load-test scripts (see demo/README.md)
│   ├── demo.ts
│   ├── load-test.ts
│   └── README.md
│
└── app.ts                                  ← Express app + route registration
```

---

## Database Schema

Eight tables. Migrations run in dependency order via `typeorm migration:run`. Migrations are written by hand — not generated — to ensure the schema is fully understood and intentional.

### Core tables

| Table | Key columns |
|---|---|
| `customers` | `id serial PK`, `name text`, `email text unique` |
| `products` | `id serial PK`, `name text`, `sku text unique`, `price numeric(10,2)`, `description text`, `tax_code text` |
| `warehouses` | `id serial PK`, `name text`, `address text`, `latitude double precision`, `longitude double precision` |
| `inventories` | `warehouse_id FK`, `product_id FK`, `quantity int ≥ 0` — unique on `(warehouse_id, product_id)` — **physical stock only, never written on the order path** |

### Order tables

| Table | Key columns |
|---|---|
| `orders` | `id serial PK`, `customer_id FK`, `warehouse_id FK`, `warehouse_name text`, `warehouse_address text`, `shipping_address jsonb`, `subtotal numeric(10,2)`, `tax_amount numeric(10,2)`, `total numeric(10,2)`, `status text CHECK('PENDING_PAYMENT','CONFIRMED','FAILED')`, `payment_reference text`, `idempotency_key text unique`, `reservation_group_id uuid unique`, `receipt_snapshot jsonb`, `created_at timestamptz` |
| `order_items` | `id serial PK`, `order_id FK`, `product_id FK`, `quantity int`, `unit_price numeric(10,2)`, `product_name text`, `product_sku text`, `product_description text`, `tax_code text`, `tax_rate numeric(5,4)`, `tax_amount numeric(10,2)` |

### Reservation table

| Table | Key columns |
|---|---|
| `inventory_reservations` | `id serial PK`, `inventory_id FK`, `reservation_group_id uuid`, `quantity int`, `status text CHECK('active','confirmed','released')`, `expires_at timestamptz`, `created_at timestamptz` |

Reservations are grouped by `reservation_group_id`, a server-generated UUID, **not** by `order_id`. The key is generated at the start of the request — before the order row exists — so inventory can be reserved before the order (and before the tax call). The order carries the same `reservation_group_id` for the reverse lookup. This decouples reservation from order creation: a reservation is an inherently pre-order transient, and one whose order is never created is simply released by the expiry job. See [Inventory Management System](#inventory-management-system) for why reserving early matters.

**Critical index on `inventory_reservations`:**

```sql
CREATE INDEX idx_inventory_reservations_availability
ON inventory_reservations (inventory_id, status, expires_at)
WHERE status IN ('active', 'confirmed');
```

This index is load-bearing. The availability query runs on every warehouse selection check and aggregates across all reservations that still hold stock per product. Without it, the query degrades linearly as the table grows. The partial `WHERE status IN ('active', 'confirmed')` clause keeps the index small — only `released` rows are excluded, since they no longer count against availability.

Seeds provide 1 customer, 3 products with different tax codes, 2 warehouses with varying inventory for test coverage.

---

## Temporal Data Integrity — Order Snapshots

An order is a **point-in-time contract** with the customer. Product prices, names, tax codes, and warehouse details can change at any time; the order must always reflect what the customer actually saw and paid.

### Two-layer snapshot strategy

**Layer 1 — `order_items` denormalized (queryable)**

Every `order_item` stores a snapshot of the product fields at the moment of purchase. The `product_id` FK is kept for analytics and traceability only — never used to reconstruct what the customer bought.

```
order_items:
  product_id          ← FK for analytics / traceability only
  unit_price          ← snapshot: price at time of purchase
  product_name        ← snapshot: name at time of purchase
  product_sku         ← snapshot: SKU at time of purchase
  product_description ← snapshot: description at time of purchase
  tax_code            ← snapshot: tax code at time of purchase
  tax_rate            ← snapshot: rate applied at time of purchase
  tax_amount          ← tax applied to this line item
```

This layer is SQL-queryable with standard indexed queries: revenue reports, SKU-level sales breakdowns, tax reporting, dispute resolution.

**Layer 2 — `receipt_snapshot jsonb` on `orders` (immutable display)**

A serialized, self-contained representation of the full order at creation time, used exclusively to render receipts to the customer without joins:

```json
{
  "items": [
    {
      "product_id": 1,
      "product_name": "Organic Apples",
      "sku": "APPL-ORG-1KG",
      "quantity": 2,
      "unit_price": 5.00,
      "subtotal": 10.00,
      "tax_code": "GROCERY",
      "tax_rate": 0.00,
      "tax_amount": 0.00
    },
    {
      "product_id": 2,
      "product_name": "Wireless Headphones",
      "sku": "ELEC-WH-001",
      "quantity": 1,
      "unit_price": 80.00,
      "subtotal": 80.00,
      "tax_code": "ELECTRONICS",
      "tax_rate": 0.21,
      "tax_amount": 16.80
    }
  ],
  "warehouse": {
    "id": 3,
    "name": "North Warehouse",
    "address": "742 Evergreen Terrace, Springfield"
  },
  "subtotal": 90.00,
  "tax_amount": 16.80,
  "total": 106.80,
  "pricing_at": "2026-06-09T14:30:00Z"
}
```

This blob is written once and never updated. It is the source of truth for customer-facing receipts, PDF generation, and payment dispute evidence.

`warehouse_name` and `warehouse_address` are also snapshotted as flat columns on `orders` for the same reason.

### Stored vs virtual fields for order totals

`subtotal`, `tax_amount`, and `total` on `orders` are stored columns, not virtual computed fields, for two reasons:

- **Performance.** A virtual field recomputes on every read via a JOIN + SUM across `order_items`. For an order history page showing 50 orders, that is 50 aggregations on every load.
- **Audit immutability.** The stored `total` is the legal record of what the customer was charged. Even if a bug or migration ever touched `order_items`, the charged amount remains correct and auditable.

A DB-level consistency constraint asserts `total = subtotal + tax_amount` at write time, catching any discrepancy at the source.

---

## Inventory Management System

When multiple orders arrive simultaneously for the same product, the system must guarantee that available stock is never oversold — each order must see an accurate count of what is actually free to reserve, and two concurrent orders must not both claim the last unit. The naive approach is to lock the inventory row for the duration of each order transaction, but this serializes all concurrent orders for the same SKU and collapses throughput under load. The reservation table approach below solves both correctness and concurrency by making inventory claims append-only writes rather than row updates.

### Why not `SELECT FOR UPDATE`

`SELECT FOR UPDATE` holds a write lock on the inventory row for the entire transaction duration. Under high concurrency on the same product, all competing transactions queue on that single lock. With 100 simultaneous orders for the same SKU and ~300ms per transaction, throughput collapses to ~3 orders/second and P99 latency reaches 30 seconds.

### Reservation table approach

Inventory reservation is append-only. Instead of updating a hot row, the system inserts a new `inventory_reservations` row per item. No row-level write contention; the DB processes parallel inserts independently.

**Available inventory** is a derived read-only value computed in `InventoryRepository`:

```sql
SELECT
  i.quantity - COALESCE(SUM(r.quantity), 0) AS available
FROM inventories i
LEFT JOIN inventory_reservations r
  ON  r.inventory_id = i.id
  AND (
    (r.status = 'active' AND r.expires_at > NOW())
    OR r.status = 'confirmed'
  )
WHERE i.warehouse_id = :warehouseId
  AND i.product_id   = :productId
GROUP BY i.id, i.quantity
```

Both **`active`** and **`confirmed`** reservations subtract from available stock. This is essential: an order is never decremented from `inventories.quantity` on the order path. When payment succeeds and a reservation flips `active → confirmed`, it must keep counting against availability — otherwise the confirmed stock would reappear as available the moment an order is paid, causing oversell. The `expires_at > NOW()` filter applies only to `active` reservations; `confirmed` reservations never expire and count until the goods physically ship.

`inventories.quantity` represents **physical stock** and is only written during receiving/restocking and by the `fulfillReservations` job — never on the order path. A `confirmed` reservation continues to hold stock against availability until that job decrements `quantity` and flips the reservation to `released` (see [Background Jobs](#background-jobs)).

### Reservation lifecycle

```
reservationGroupId = randomUUID()   ← before any order row exists
       ↓
INSERT reservation (reservation_group_id, status='active', expires_at=NOW()+10min)
       ↓
 Order inserted (carries reservation_group_id), then payment attempted
       ↓
  ┌────┴────┐
success   failure
  ↓           ↓
status=    status=
confirmed  released
(by reservation_group_id)
```

Reservations that reach `expires_at` while still `active` are released by the `expireReservations` background job. `confirmed` reservations are terminal on the order path — they do not expire and continue to subtract from available stock until the `fulfillReservations` job decrements physical `inventories.quantity` and flips them to `released`.

### Lock scope and timing

A **PostgreSQL advisory transaction lock** is acquired **per `(warehouseId, productId)` pair** and held from the availability check through the reservation insert — both operations run inside one DB transaction. This is what makes the reservation atomic: without holding the lock during the check, two concurrent requests can both observe sufficient inventory before either inserts, resulting in oversell.

```
BEGIN
  SELECT pg_advisory_xact_lock(warehouseId, productId)  ← blocks until the pair's lock is free
  ↓
  run availability query          ← inside lock
  ↓
  re-verify quantities sufficient ← inside lock
  ↓
  insert reservation row          ← inside lock
COMMIT                            ← lock released atomically here (or on ROLLBACK)
```

**Why advisory locks over a Redis `SET NX` lock.** The first design used a Redis distributed lock with a TTL. Two problems under load: (1) if the DB work ran longer than the TTL, the key auto-expired mid-operation and a second holder could enter → oversell; (2) the lock was a single non-blocking attempt, so under a burst of N concurrent requests for the same SKU, one won and the other N−1 failed outright. The advisory lock has no TTL — it is released by the database exactly at commit/rollback, never mid-flight — and `pg_advisory_xact_lock` **blocks and queues** rather than failing, so concurrent same-SKU requests serialize and all succeed (up to available stock) instead of erroring. The two-int4 overload maps the `(warehouseId, productId)` natural key directly with no hashing.

Locks are acquired in ascending `productId` order across all items in an order to prevent deadlocks when an order touches multiple products. Two orders for different products in the same warehouse never block each other.

### Throughput comparison

| Strategy | Throughput (same SKU) | P99 latency (100 concurrent orders) |
|---|---|---|
| `SELECT FOR UPDATE` | ~3–4 orders/sec | ~25–30 seconds |
| Reservation table | ~100–200 orders/sec | ~200–400ms |

---

## Tax Gateway

Tax is calculated per line item after the subtotal is computed and before the payment gateway is called. The `shippingAddress` determines the jurisdiction; the `tax_code` on each product determines the applicable rate within that jurisdiction. The server derives the jurisdiction from the address — the client never sends it.

**`src/gateways/TaxGateway.ts`**

```ts
TaxGateway.calculate({
  shippingAddress: string
  items: {
    taxCode: string    // e.g. 'GROCERY', 'ELECTRONICS', 'EXEMPT'
    amount:  number    // line item subtotal
  }[]
}): Promise<{
  totalTaxAmount: number
  breakdown: {
    taxCode:   string
    taxRate:   number   // e.g. 0.21
    taxAmount: number
  }[]
}>
```

Mock returns deterministic rates per tax code (e.g. `GROCERY → 0.00`, `ELECTRONICS → 0.21`, `STANDARD → 0.10`). Throws `TaxCalculationError` for unknown tax codes (test hook). Real implementation swaps this for a TaxJar or Avalara call.

Circuit breaker flag `circuit:tax` checked before every call. Maps to `503` if open.

---

## Layer Responsibilities in Detail

### Route Layer — `src/routes/orders.ts`

`POST /orders` middleware pipeline:

```
1. rateLimiter middleware       ← 5 req / 60s sliding window, keyed on client IP (req.ip; needs `trust proxy` behind a load balancer)
2. Zod schema validation        ← rejects malformed requests before hitting controller
3. orderController.createOrder  ← delegates to controller
```

The route layer is purely infrastructural: rate limiting, validation, delegation. It has no knowledge of business logic or database structure.

### Controller Layer — `src/controllers/orderController.ts`

The controller is the natural home for idempotency — it is the layer that decides whether to invoke business logic at all, it already has the validated DTO (including `idempotencyKey`), and it is where the response is first constructed, making it clean to store immediately after `OrderService` returns.

```
1. Claim idempotency key        ← IdempotencyRepository.claim(key, 24h)  ← atomic SET NX "pending"
   - not claimed → get(key): cached response present → return it; else 409 (still in flight)
2. Call OrderService.createOrder(dto)
3. Store response               ← IdempotencyRepository.set(key, response, 24h)  ← overwrites "pending"
   - on any throw → IdempotencyRepository.release(key) so the key can be retried, then rethrow
4. Return 201 { orderId, subtotal, taxAmount, total, warehouseId, status }
```

The claim is an atomic `SET NX`, not a read-then-write: only the first of N concurrent requests with the same key wins, closing the race where two requests both miss the cache and both charge. The controller never calls repositories other than `IdempotencyRepository`. All business logic flows through `OrderService`.

### Service Layer — `src/services/`

`OrderService.ts` is the **Facade** — the single entry point called by the controller:

```
1. Fetch products via ProductRepository; compute subtotal server-side
2. WarehouseSelectionService.findClosest(items, shippingAddress)
3. reservationGroupId = randomUUID()
4. ReservationService.createReservations(warehouseId, items, reservationGroupId)  ← reserve EARLY
   ── from here, any failure releases by reservationGroupId (compensating transaction) ──
5. Check tax circuit breaker → throw CircuitOpenError (503) if open
6. TaxGateway.calculate(shippingAddress, items with tax codes); total = subtotal + taxAmount
7. OrderRepository.insert(order with reservationGroupId) + OrderItemRepository.insertMany(items) with full snapshots
8. Check payment circuit breaker → throw CircuitOpenError (503) if open
9. withRetry(() => PaymentGateway.charge(total, ...), 2, 500ms)
10. On success: ReservationService.confirmReservations(reservationGroupId)
                OrderRepository.updateStatus(orderId, 'CONFIRMED')
11. On failure: ReservationService.releaseReservations(reservationGroupId)  ← compensating transaction
                OrderRepository.updateStatus(orderId, 'FAILED')  ← only if the order was created
                if release fails → deadLetterQueue.send(...)
```

Inventory is reserved at step 4 — before the tax gateway call and before any order write. Reservations key on `reservationGroupId` rather than `orderId`, so they no longer depend on an existing order row. Failing fast here avoids a wasted tax call and order insert when stock is unavailable, and means a failed reservation never leaves an orphaned `PENDING_PAYMENT` order. The order in step 7 is created only once inventory is secured, and the compensating release on step 11 keys by `reservationGroupId` — marking the order `FAILED` only if it had already been inserted.

**`ReservationService.createReservations`** — for each `(warehouseId, productId)` pair, in ascending `productId` order, each within its own transaction:

```
1. BEGIN transaction (opened by the service; manager passed to each repository call)
2. InventoryRepository.lockForReservation(warehouseId, productId, manager)  ← blocks until the pair's lock is free
3. InventoryRepository.getAvailable(...)  ← inside lock, on the transaction's manager
4. Throw InsufficientInventoryError if quantity < requested  ← inside lock (rolls back → lock released)
5. ReservationRepository.insert(reservation_group_id, status='active', expires_at=NOW()+10min, manager)  ← inside lock
6. COMMIT                                                     ← lock released atomically
```

The raw `SELECT pg_advisory_xact_lock(...)` lives in `InventoryRepository.lockForReservation` — all SQL stays in the data layer; the service only opens the transaction and orchestrates the calls. The advisory lock is held across steps 3–5 and released by the database exactly at COMMIT/ROLLBACK. This is what prevents two concurrent requests from both observing sufficient inventory before either inserts, and — unlike a TTL-bounded Redis lock — it can never expire mid-operation. The reservation row carries `reservation_group_id` (passed in by `OrderService`), not an `order_id`.

**`GeocodeService`** — cache wrapper with circuit breaker scoped to the gateway call:

```
1. Check Redis cache (key: geocode:<sha256(address)>)
2. If cache hit → return immediately (circuit breaker irrelevant)
3. If cache miss:
   a. Check circuit:geocoding flag → throw CircuitOpenError (503) if open
   b. Call GeocodingGateway.geocode(address)
   c. Write result to cache (TTL 30 days)
   d. Return coordinates
```

The circuit breaker is checked only on a cache miss, immediately before the gateway call. A cache hit bypasses it entirely — no external call is made, so there is nothing to protect against.

Sub-services called internally by `OrderService`:

- `WarehouseSelectionService` — queries available inventory from `InventoryRepository`, geocodes address via `GeocodeService`, scores by haversine distance
- `ReservationService` — per-product advisory-locked transaction (check + insert within the lock window), inserts reservations
- `GeocodeService` — Redis cache wrapper around `GeocodingGateway`, circuit breaker on cache miss only

### Data Layer — `src/repositories/`

All TypeORM queries are isolated here. Services call repositories; repositories never call services.

| Repository | Responsibilities |
|---|---|
| `OrderRepository` | insert order, updateStatus, findByIdempotencyKey, findStalePending |
| `OrderItemRepository` | insertMany with full snapshots |
| `ProductRepository` | findByIds (price, name, sku, tax_code lookup at order time) |
| `WarehouseRepository` | findAll with coordinates |
| `InventoryRepository` | available inventory query (physical − active/confirmed reservations); lockForReservation (advisory lock); decrementQuantity (fulfillment) |
| `ReservationRepository` | insert (by reservation_group_id), confirmByGroupId, releaseByGroupId, releaseExpiredActive, findConfirmedGrouped |
| `IdempotencyRepository` | Redis claim (SET NX) / get / set EX / release (no TypeORM — Redis-backed) |

---

## Compensating Transactions + Dead Letter Queue

Payment and reservation confirmation are separate operations. If payment succeeds but `confirmReservations` fails, the system must not leave reservations permanently `active` consuming stock for an order that will never be fulfilled.

```
PaymentGateway.charge()
  ↓ success
confirmReservations()
  ↓ failure
releaseReservations()          ← compensating transaction
  ↓ failure
deadLetterQueue.send({         ← triggers alert, manual investigation
  order_id,
  failure_stage: 'post_payment_release',
  payment_reference
})
```

The dead-letter queue is a BullMQ queue with no automatic retry. An alert fires on any enqueue. This ensures no order is silently lost in a partially-failed state.

---

## Idempotency

The `idempotency_key` column on `orders` has a `UNIQUE` constraint as a DB-level safety net. The primary idempotency check happens in the controller layer via Redis (TTL 24h) — it short-circuits before business logic is invoked, returning the original response without re-executing any business logic.

The check is an **atomic claim**, not a read-then-write. The controller calls `IdempotencyRepository.claim(key)` which does `SET key "pending" NX EX 24h`:

- **First request wins the claim** → proceeds through `OrderService`, then overwrites `"pending"` with the real response via `set()`. If it throws, `release(key)` deletes the claim so the client can retry.
- **A duplicate or concurrent request fails the claim** → it reads `get(key)`: a completed original returns its cached response; a still-in-flight request (only the `"pending"` sentinel, which `get()` treats as no-response) returns `409 Conflict`.

This closes the check-then-set race where two simultaneous requests both read `null` from a plain GET and both proceed to reserve inventory and charge the card. Any duplicate submission due to client retry, network timeout, or double-click is handled transparently.

---

## Circuit Breakers

Redis-backed boolean flags, one per external dependency. Checked in the service layer immediately before each gateway call.

| Flag | Where checked | Behaviour on cache hit |
|---|---|---|
| `circuit:geocoding` | `GeocodeService` — after cache miss, before gateway call | Bypassed — no external call made |
| `circuit:payment` | `OrderService` — before `PaymentGateway.charge` | N/A |
| `circuit:tax` | `OrderService` — before `TaxGateway.calculate` | N/A |

When a flag is `true` (circuit open), a `CircuitOpenError` is thrown, mapping to `503 Service Unavailable`. Flags are toggled manually via ops tooling or automatically after a configurable error threshold.

---

## Type Safety

TypeScript strict mode throughout. No `any`. All external inputs validated with Zod at the route layer and narrowed to inferred types before reaching the controller.

Order status uses a discriminated union — invalid states are unrepresentable at compile time:

```ts
type Order =
  | { status: 'PENDING_PAYMENT'; reservations: Reservation[] }
  | { status: 'CONFIRMED';       confirmedAt: Date; paymentReference: string }
  | { status: 'FAILED';          reason: string }
```

Accessing `.paymentReference` on a `PENDING_PAYMENT` order is a compile-time error.

---

## Observability

**Structured logging** via pino. Every significant event emits a JSON log line with business context. No `console.log` anywhere in the codebase.

```json
{
  "timestamp": "2026-06-09T14:30:00Z",
  "level": "info",
  "event": "order_confirmed",
  "customer_id": 123,
  "order_id": "ord-456",
  "warehouse_id": 3,
  "subtotal": 90.00,
  "tax_amount": 16.80,
  "total": 106.80,
  "fulfillment_latency_ms": 284
}
```

**Metrics** (exported to Prometheus / StatsD):

| Metric | Type | Description |
|---|---|---|
| `orders_created_total` | Counter | Total confirmed orders |
| `orders_failed_total` | Counter | Total failed orders, labeled by `reason` |
| `order_fulfillment_latency_ms` | Histogram | End-to-end order creation latency |
| `warehouse_reservation_conflicts_total` | Counter | Re-verify failures under lock |
| `payment_gateway_error_rate` | Gauge | Rolling error % on payment calls |
| `tax_gateway_error_rate` | Gauge | Rolling error % on tax calls |
| `inventory_reservations_expired_total` | Counter | Reservations released by expiry job |
| `inventory_fulfilled_total` | Counter | Confirmed reservations rolled into physical stock by the fulfillment job |
| `dead_letter_queue_depth` | Gauge | Unresolved compensating transaction failures |

---

## Background Jobs

**`src/jobs/expireReservations.ts`** — BullMQ repeatable, every 1 minute. Set-based — both steps run in one transaction, no per-row loop.

```
1. ReservationRepository.releaseExpiredActive()
   → UPDATE inventory_reservations SET status='released'
     WHERE status='active' AND expires_at < NOW()
     RETURNING reservation_group_id        ← one statement, returns affected groups
2. OrderRepository.failPendingByGroupIds(distinct groupIds)
   → set any still-PENDING_PAYMENT order on those groups to FAILED in one statement
     (a reservation whose order was never created simply releases — nothing to fail)
3. Emit metric: inventory_reservations_expired_total++
```

**`src/jobs/fulfillReservations.ts`** — BullMQ repeatable, nightly at 03:00. Rolls `confirmed` reservations into physical stock so they stop accumulating: the availability query sums every `confirmed` row, so without this a hot SKU would build unbounded rows and the query would degrade linearly. It's not time-sensitive (a `confirmed` reservation keeps holding stock until it runs) but it takes write locks on inventory rows, so it runs in a low-traffic window to keep that contention off the hot order path.

```
1. ReservationRepository.findConfirmedGrouped({ limit: 500 })
   → SELECT inventory_id, SUM(quantity), ARRAY_AGG(id)
     FROM inventory_reservations WHERE status='confirmed'
     GROUP BY inventory_id LIMIT 500       ← batched so a backlog drains over several runs
2. For each inventory group, in its own transaction:
   a. InventoryRepository.decrementQuantity(inventory_id, SUM)   ← physical stock -= confirmed total
   b. UPDATE inventory_reservations SET status='released'
      WHERE id = ANY(ids) AND status='confirmed'                 ← release exactly the summed rows
3. Emit metric: inventory_fulfilled_total++
```

The decrement and release commit atomically per inventory row. The `CHECK (quantity >= 0)` constraint guards against over-decrement (the transaction rolls back, leaving the rows `confirmed` for the next run), and the `AND status='confirmed'` guard makes overlapping runs safe. Per-inventory transactions keep lock windows short instead of holding 500 rows for the whole batch.

**`src/jobs/reconcileOrders.ts`** — BullMQ repeatable, every 5 minutes.

```
1. OrderRepository.findStalePending()
   → SELECT WHERE status='PENDING_PAYMENT' AND created_at < NOW() - INTERVAL '5 minutes'
2. For each: PaymentGateway.getStatus(payment_reference)
   - 'succeeded' → ReservationService.confirmReservations + updateStatus('CONFIRMED')
   - 'failed'    → ReservationService.releaseReservations + updateStatus('FAILED')
   - 'unknown'   → log warn, skip (retried next run)
```

---

## Demo & Load Testing

Two runnable scripts live under `src/demo/`, both driving a live server over HTTP (no test doubles):

- **`demo.ts`** (`npm run demo`) — a guided walkthrough of 8 scenarios with colour-coded pass/fail output: happy path with mixed tax codes, warehouse routing, idempotency, payment declined, insufficient inventory, the payment and tax circuit breakers, and a concurrent mini-burst against a single SKU.
- **`load-test.ts`** (`npm run load-test`) — concurrent stress tests: a throughput run (100 simultaneous orders against ample stock, all confirm) and an oversell-prevention run (60 simultaneous orders against 50 units — confirms ≤ 50, proving the advisory lock holds under load).

For setup, prerequisites, and a per-scenario breakdown, see **[`src/demo/README.md`](src/demo/README.md)**.

---

## Verification

```bash
# DB setup
createdb orders_dev
npm run migration:run
npm run seed

# Start Redis
redis-server --daemonize yes

# Run server
npm run dev

# Smoke test
curl -X POST http://localhost:3000/orders \
  -H 'Content-Type: application/json' \
  -d '{
    "orderToPlace": {
      "customerId": 1,
      "shippingAddress": "123 Main St, Seattle WA",
      "items": [
        { "productId": 1, "quantity": 2 },
        { "productId": 2, "quantity": 1 }
      ],
      "cardNumber": "4111111111111111",
      "idempotencyKey": "test-001"
    }
  }'

# Run tests
npm test

# Demo walkthrough + load tests (see src/demo/README.md)
npm run demo
npm run load-test
```

---

## Implementation

List of tasks to track the implementation progress:

- [x] Project scaffold — `package.json`, `tsconfig.json` (strict mode), ESLint, Prettier, Vitest config, `src/config/env.ts`
- [x] TypeORM setup — `src/db/dataSource.ts`, base entity config, migration runner script
- [x] Migration: `customers`, `products`, `warehouses`, `inventories`
- [x] Migration: `orders`, `order_items`
- [x] Migration: `inventory_reservations` with partial index on `(warehouse_id, product_id, status, expires_at)`
- [x] Seed data — 1 customer, 3 products (mixed tax codes), 2 warehouses with inventory
- [x] TypeORM entities — `Customer`, `Product`, `Warehouse`, `Inventory`, `InventoryReservation`, `Order`, `OrderItem`
- [x] Repositories — `OrderRepository`, `OrderItemRepository`, `ProductRepository`, `WarehouseRepository`, `InventoryRepository`, `ReservationRepository`, `IdempotencyRepository`
- [x] Utilities — `haversine.ts`, `retry.ts`, `logger.ts` (pino)
- [x] Mock gateways — `GeocodingGateway`, `PaymentGateway` (with `PaymentDeclinedError` hook), `TaxGateway` (with `TaxCalculationError` hook)
- [x] `GeocodeService` — Redis cache wrapper; circuit breaker checked on cache miss only
- [x] `WarehouseSelectionService` — eligible warehouse query + haversine sort
- [x] `ReservationService` — per-product advisory-locked transaction (check + insert within the lock window), confirm, release
- [x] `OrderService` (Facade) — orchestrates subtotal, warehouse selection, tax, reservation, payment, compensation, dead-letter queue
- [x] Middleware — `rateLimiter.ts` (per-IP sliding window), `circuitBreaker.ts` (Redis flag reader)
- [x] Controller — `orderController.ts` with atomic idempotency claim + store
- [x] Route — `POST /orders` pipeline (rate limit → validate → controller)
- [x] Express app wiring — `app.ts`, `src/main.ts` server entry
- [x] BullMQ job: `expireReservations` (every 1 min)
- [x] BullMQ job: `fulfillReservations` (nightly at 03:00)
- [x] BullMQ job: `reconcileOrders` (every 5 min)
- [x] Unit tests — `haversine`, `WarehouseSelectionService`, `ReservationService`, `TaxGateway`, jobs, controller
- [x] Demo + load-test scripts — `src/demo/` (see `src/demo/README.md`)
